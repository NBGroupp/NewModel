...
def run_epoch(session, model, data, train_op, is_training, batch_size, step_size, char_set, file,
    ...
    for step in range(step_size):
        ...
        original_word = int(y[0])
        PROOFREAD_BIAS = 0.1#0.01
        if(outputs[0][original_classes[0]] >= PROOFREAD_BIAS * outputs[0][output_classes[0]]):
            classes[0] = original_word
        ...
        # 统计评价参数
        statistics_evaluation(classes, target_index, y, outputs,output_classes, original_classes)
