...
class Proofreading_Model(object):
    def __init__(self, is_training, batch_size, num_steps , word2vec_mat):
        """
        ...
        embedding = tf.constant(word2vec_mat)
        ...
        weight = tf.get_variable("weight", [2 * HIDDEN_SIZE, HIDDEN_SIZE])
        ...
        loss = tf.square(self.logits - targets)
...
def run_epoch(session, model, data, train_op, is_training, batch_size, step_size, char_set, file,
    ...
    #初始化数据
    [pre_state,fol_state] = session.run([model.pre_initial_state, model.fol_initial_state])
    ...
    for step in range(step_size):
        ...
        # print(y)
        #lstm迭代计算
        cost, pre_state, fol_state, outputs, _, _, ave_cost_op\
