...
class Proofreading_Model(object):
    def __init__(self, is_training, batch_size):
        ...
        with tf.variable_scope('Pre') as scope:
            pre_cell = IndRNNCell(num_units=PRE_CONTEXT_HIDDEN_SIZE, recurrent_max_abs=RECURRENT_MAX,
                                  input_kernel_initializer=input_init, recurrent_kernel_initializer=recurrent_init)
            if is_training:
            ...
            pre_outputs, pre_states = tf.nn.dynamic_rnn(pre_lstm_cell, pre_input,
                                                        sequence_length=self.pre_input_seq_length,dtype=tf.float32)
            pre_outputs = pre_states
            self.pre_final_state = pre_states  # 上文LSTM的最终状态

        # fol_context_model
        with tf.variable_scope('Fol') as scope:
            fol_cell = IndRNNCell(num_units=PRE_CONTEXT_HIDDEN_SIZE, recurrent_max_abs=RECURRENT_MAX,
                                  input_kernel_initializer=input_init, recurrent_kernel_initializer=recurrent_init)
            if is_training:
