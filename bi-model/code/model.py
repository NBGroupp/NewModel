...
class Proofreading_Model(object):
    def __init__(self, is_training, batch_size):
        ...
        self.input_seq_length = tf.placeholder(tf.int32, [batch_size,])
        ...
        with tf.variable_scope('bilstm') as scope:       
            fw_cell = tf.contrib.rnn.BasicLSTMCell(num_units=PRE_CONTEXT_HIDDEN_SIZE, forget_bias=0.0,
                                                state_is_tuple=True)
            bw_cell = tf.contrib.rnn.BasicLSTMCell(num_units=PRE_CONTEXT_HIDDEN_SIZE, forget_bias=0.0,
                                                state_is_tuple=True)
            ...
            fw_cell = tf.contrib.rnn.MultiRNNCell([fw_cell] * PRE_CONTEXT_NUM_LAYERS, state_is_tuple=True)
            bw_cell = tf.contrib.rnn.MultiRNNCell([bw_cell] * PRE_CONTEXT_NUM_LAYERS, state_is_tuple=True)

            inputs = tf.nn.embedding_lookup(embedding, self.inputs)  # 将原本单词ID转为单词向量。
            if is_training:
            ...
            output = tf.concat([states[0][0][-1], states[1][0][-1]], axis=-1) 
            self.final_state = states  #上文LSTM的最终状态
