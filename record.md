...
2.发现并没有解决模型保存后再读取的问题！！！！！丧气。小数据是可以的，不知道原因
3.清洗好的数据在跑的时候发现溢出了，运行到13轮就停止了，发现是因为写入文件的数据太多，改为2000步写一次了,效果和之前差不多
...
2.发现模型效果不佳，尝试更大的隐藏层维度和更大的深度

3.查找阅读理解相关模型，尝试使用形近字进行填空

### 结果简述

1.下载相关latex模板，论文框架基本完成

2.在机房配置了两台机器，可以同时跑

3.之前的模型效果是由于data2数据有误，第一行为空，数据串行，因此预测成功的词的信息中都已经包含了该词，真正的模型正确率不超过25%，需要再改进模型

## 2018年1月28日，记录人：刘明桓

### 任务摘要

1.初步决定参考阅读理解相关模型，使用形近字作为候选字。

2.阅读论文，明确模型细节，搭建模型

### 结果简述

1.认为模型在有候选字的情况下会得到更好的效果，但是这样做局限了模型的功能
